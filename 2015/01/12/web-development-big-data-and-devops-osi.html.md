...
[MongoDB](http://www.mongodb.org/) - NoSQL database with dynamic schema, in-built aggregation, mapreduce, JSON style document, auto-sharding, extensive query mechanism and high availability.
...
[Node.js](http://nodejs.org/) - A server side javascript framework with event based programming and single threaded (non blocking I/O with help of request queue).

[Sails.js](http://sailsjs.org/) - MEAN Stack provisioner to develop applications quickly.
...
Dushyant Min spoke about the way Facebook optimised the PHP code base to deliver better performance when they supposed to handle a massive growth of users. Earlier there were compilers HipHop for PHP(HPHPc) or HPHPi(developer mode) to convert the php code to C++ binary and executed to provide the response. After sometime, Facebook developed a new compilation engine called HipHop Virtual Machine([HHVM](http://hhvm.com/)) which uses Just-In-Time(JIT) compilation approach and converts the code to HipHop ByteCode(HHBC). Both Facebook’s production and development environment code runs over HHVM.
...
Also PHP code can be migrated to [Hack](http://hacklang.org/) by changing the <?php tag to <?hh and there are some converters (hackficator) available for code migration. Both PHP and Hack provide almost the same performance on the HHVM engine, but Hack has some additional developer-focussed features.
...
[Elasticsearch](http://www.elasticsearch.org/) - Open source full text search and analytics engine

[Log Stash](http://logstash.net/) - Open source tool for managing events and logs which has following steps to process the logs

[Kibana](http://www.elasticsearch.org/overview/kibana) - seamlessly works with Elasticsearch and provides elegant user interface with various types of graphs
