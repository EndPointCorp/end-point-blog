...
Now that the problem is fixed, how do you prevent it from happening again? First, you should use the ['tail_n_mail'](https://bucardo.org/wiki/Tail_n_mail) program to monitor your Postgres log files, so that the moment the archive_command starts failing, you will receive an email and can deal with it right away. Making sure your pg_xlog partition has plenty of space is a good strategy as well, as the longer it takes to fill up, the more time you have to correct the problem before you run out of disk space.
