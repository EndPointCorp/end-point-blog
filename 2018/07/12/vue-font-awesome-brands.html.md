---
title: Vue, FontAwesome, and Brand Icons 
...
[FontAwesome](https://fontawesome.com) and [Vue](https://www.vuejs.org/) are both great technologies.  Here I detail overcoming some issues when using them together in the hopes of saving others future debugging time.
...
After turning up dry for a run to the Google well and scanning the docs, I determined that this must come down to a difference in the prefix; since the icons that worked were being imported from the `free-solid-svg-icons` library, it would seem that that was the source of the `fas` prefix.  Since the non-working icons were coming from the `free-brands-svg-icons` library it to reason that somehow passing in a prefix parameter of `fab` would work.
