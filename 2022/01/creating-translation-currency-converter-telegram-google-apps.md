...
date: 2022-01-13
title: "Creating Translation and Currency Converter Bot with Telegram and Google Apps Script"
github_issue_number: 
tags:
- google-apps-script
---

In the previous [post](/blog/2021/11/forwarding-google-forms-responses-to-api/) written by Afif, he elaborated on how to use Google Apps Script with Google Form. Coincidentally last year I learned a bit on how to use Google Apps Script with Telegram Bot as a Personal Ledger Tool written by [Mars Escobin](https://medium.com/@mars_escobin/telegram-inline-keyboards-using-google-app-script-f0a0550fde26).The base code that she wrote can be referred [here](https://github.com/mariannetrizha/budgetter/blob/master/budgetter_bot.js)
...
In this writing I will share a bit on how to adapt Mars’ code and use Telegram Bot to get the input from the user and let Google Apps Script to call Google’s cloud based services (translation and finance) and later return the outputs to the user. 

(/blog/2021/12/creating-translation-and-currency-converter-bot-with-telegram-and-google-app-script/botfather.jpg)

The initial process of having a Telegram bot can be referred [here](https://core.telegram.org/bots). After we had received the Telegram's API key then we could use it inside our Google Apps Script editor. 
...
var webAppUrl = "<insert your webAppURL which is generated from Google Apps Script UI over here>";
...
var ssId = "<insert your webAppURL which is generated from Google Apps Script UI over here>";
...
### Special Notes
Throughout the process that I learned to use the Google Apps Script, I found several ways for us to refer to spreadsheet that we want deal inside the code. For example we can use:
