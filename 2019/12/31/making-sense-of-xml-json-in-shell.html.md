...
<rss version="2.0"
        ...
        <item>
            <title>Imsak</title>
            <description>05:53:00</description></item>
...
Using jq, we can get the info flow into the pipe and parse it to the terminal.
...
sumber=$(curl -s "https://api.exchangeratesapi.io/api/latest?base=$sourcemoney" | jq . | grep -i $target | awk -F ':\|,' {'print $2'} 2>/dev/null)                                                                 
