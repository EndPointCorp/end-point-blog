...
So for example, let's suppose we have simple a ```GetUserIDByEmail()``` function that returns the ID of the user that matches an email passed by parameter, and that function uses an asyncronous query to the database to find that user:
