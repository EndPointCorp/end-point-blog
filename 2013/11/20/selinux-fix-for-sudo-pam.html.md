...
I was just reading my co-worker Lele's blog post about [making SELinux dontaudit AVC denial messages visible](http://blog.endpoint.com/2013/11/selinux-and-need-of-talking-about.html) and realized it was likely the solution to a mystery I ran into a few days ago.
...
Somewhat as expected from the error messages we saw before, sudo is being denied permission to send a message into the kernel. Now that we have the AVC errors from the audit log it's easy to [make a local SELinux policy module](http://blog.endpoint.com/2012/05/selinux-local-policy-modules.html) to allow this.
